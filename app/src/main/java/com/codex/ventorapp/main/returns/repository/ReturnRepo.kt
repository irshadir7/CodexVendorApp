package com.codex.ventorapp.main.returns.repository
...
class ReturnRepo
@Inject
constructor(private val returnAPI: ReturnApi) {
    fun getReturnList(token: String,partner_id: String): Flow<DataState<ListPicking>> = flow {
        emit(DataState.Loading)
        try {
            val bearerToken= "Bearer $token"
            val gson = Gson()
            val response: JsonObject = returnAPI.getReturn(bearerToken,partner_id)
