package com.codex.ventorapp.main.receipt.repository
...
class ReceiptRepo
@Inject
constructor(private val receiptAPI: ReceiptApi) {
    fun getReceiptList(token: String,partner_id: String): Flow<DataState<ListPicking>> = flow {
        emit(DataState.Loading)
        try {
            val bearerToken= "Bearer $token"
            val gson = Gson()
            val response: JsonObject = receiptAPI.getReceipt(bearerToken,partner_id)
